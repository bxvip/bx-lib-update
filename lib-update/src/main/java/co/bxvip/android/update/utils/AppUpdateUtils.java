package co.bxvip.android.update.utils;
...
public class AppUpdateUtils {
    ...
    public static boolean installApp(Activity activity, File appFile) {
        try {
            ...
            if (context == null) {
                context = RePlugin.getPluginContext();
            }
            if (context == null) {
                context = activity;
            }
            ...
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                System.out.println("error:" + context.toString() + "---" + context.getPackageName() + "----" + appFile.getAbsolutePath());
